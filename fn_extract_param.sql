CREATE OR ALTER FUNCTION [jadoube].[fn_extract_param] (
    @string varchar(max),
    @separator varchar(1) = ';',
    @key varchar(max),
    @include_key bit = 0
...
Usage:
>>> [jadoube].[fn_extract_param]('server=<server>;database=<database>', DEFAULT, @key = 'database_', DEFAULT)
'<database>'
